...
- All public routes live under `src/pages/[lang]/...`, where `lang` is one of the 7 supported languages defined in `src/i18n/languages.ts`: `LANGUAGES = ['ru', 'en', 'es', 'pt', 'de', 'fr', 'br']`, with `DEFAULT_LANGUAGE = 'ru'`. The site root `/` redirects to `/ru/` (configured in `astro.config.mjs`). **Only `INDEXED_LANGUAGES = ['ru', 'en']` participate in search**: sitemap, hreflang and the language switcher list them alone, the other five render with `noindex, follow` (decision of 2026-08-24, see `docs/seo/roadmap.md` phase 2 — reverting a locale is a one-line change in `src/i18n/languages.ts`).
