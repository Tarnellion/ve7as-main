...
- Route groups: `[lang]/index.astro` (homepage index), `[lang]/[section]/index.astro` (niche archive), `[lang]/articles/[slug].astro` (article page), `[lang]/faq.astro`, `[lang]/authors/[slug].astro` (author profile), `[lang]/tools/` (index plus client-side calculators: wagering, odds converter, bookmaker margin, blackjack house edge — the blackjack one reads its verified rule deltas from `src/data/blackjack-house-edge.ts`), plus static pages `about`/`contacts`/`privacy`/`terms`/`editorial-policy`, and `src/pages/404.astro`.
